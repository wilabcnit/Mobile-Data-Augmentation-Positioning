...
def train_cgan_radio(epochs=500, batch_size=64, lr=0.0002):
    ...
    criterion = nn.MSELoss() # LSGAN (Least Squares GAN) is often more stable for regression
    ...
    for epoch in range(epochs):
        for real_cond, real_data in dataloader:
            ...
            # Generator wants D to label fake data as valid
            g_fake = D(fake_data, real_cond)
            g_loss = criterion(g_fake, valid)
            
            # Optional: Add L1 Loss to guide Generator closer to Ground Truth (Supervised cGAN)
            l1_loss = nn.L1Loss()(fake_data, real_data)
            total_g_loss = g_loss + 100 * l1_loss # Weight L1 heavily for regression accuracy
